...
static void usage(void) {
	fprintf(stderr, "usage: lich [-a|-A|-c|-n|-d|-K] [-N bytes] [-L lines] [-x] [-p] [-r] [-q] [-l] [-f] [-e detachkey] name [command|keys...]\n");
	exit(EXIT_FAILURE);
}
...
static bool create_socket_dir(struct sockaddr_un *sockaddr) {
	...
	for (unsigned int i = 0; i < countof(socket_dirs); i++) {
		...
		if (!xsnprintf(sockaddr->sun_path+dirlen, maxlen-dirlen, ".lich-%d", getpid()))
			continue;
